...
Dernière révision : **10/09/2026**, version 0.99.42 · build 144.
...
C'est une cible de prédilection ». `npm test` rend **1569 pass / 0 fail** au sens
...
`dist/index.html`, **9 310 842 octets**, 0 référence externe. Coût **+90 octets,
...
(**9 310 752**) : **feuille +0 · JavaScript +90 · balisage +0 · images +0 ·
...
9 600 000** — le lot ne fait entrer aucune ressource —, marge **289 158 octets,
3,01 %**. Version et build passent à **0.99.42 · build 144**, le suivant
...
`COMBATS_DEPLACES_PAR_MUR` porte **110 combats et 561 champs**, lus par
`JOURNAL T1` ; `COMBATS_DEPLACES_PAR_MUR_AVANT_PAQUETS` porte **125 combats et
681 champs** et devient la **CINQUIÈME** couche de `JOURNAL T1 bis`. ⚠ Elle
...
⚠⚠ **SIX FALSIFICATIONS, SIX CHUTES, ET LA SIXIÈME A ÉTÉ MUETTE AU PREMIER
RELEVÉ.** Retirer la garde aérienne de `doitSArreter` laissait `MUR T1` VERT :
...
⚠ **CINQ TESTS ENTRENT — `MUR T1` à `T5` — ET LE COMPTE PASSE DE 1 564 À 1 569.**
...
⚠⚠ **LE JUMEAU LATÉRAL DU DÉFAUT N'EST PAS TRAITÉ, ET IL SE DÉCLARE.** Le
rangement vaut pour `avancer`, donc pour le camp qui ATTAQUE ; `seDecaler` — par
où la défense des DEUX camps passe depuis le lot COLONNE — porte le même défaut
sur l'axe des COLONNES, et une défenseuse bloquée latéralement peut encore fluer
dans la case voisine. **Le brief demandait de le mesurer et de le dire, pas de le
corriger.**
...
1 skipped** (`LIMITE T8`, suspendu par Ethan le 08/09), et `npm run check` sort
en 0. `npm run build` → `dist/index.html`, **9 425 421 octets**, 0 référence
externe. Coût **+13 121 octets, SANS UN OCTET D'IMAGE NI DE SON**, mesuré poste
par poste contre le livrable rebâti dans un `git worktree` depuis `origin/main` =
`3e68d7b` (**9 412 300**) : **feuille +12 420 · JavaScript +1 051 · balisage
−350 · images +0 · audio +0**, et les cinq postes PARTITIONNENT le fichier des
deux côtés — chacun NET de ses `data:`, somme exacte sur le total avant comme
après — `data:` à **311 lignes / 306 URI** des deux côtés. Borne T10 **9 600 000**,
relevée par le lot ART-90 sur arbitrage d'Ethan et **non touchée ici** — ce lot
ne fait entrer aucune ressource —, marge **174 579 octets, 1,82 %**.
⚠⚠ **ET LA BASE A BOUGÉ SOUS LE LOT — `main` EST PASSÉ DE `14dd4ac` À `3e68d7b`
PENDANT SON EXÉCUTION**, Ethan ayant fusionné SON-ET-ARRIVÉE (#126),
RÈGLES-DE-CARTE (#125) puis ART-90 (#124). Le lot est donc REMESURÉ contre la
base neuve, poste par poste, plutôt que recopié : version et build passent à
**0.99.39 · build 141**, le suivant disponible — **`main` avait déjà pris
0.99.38 · build 140**, et les deux lots auraient porté le MÊME numéro pour deux
livrables différents, ce que l'enveloppe Android lit par `config.build`.
⚠⚠ **ET LE JAVASCRIPT PERD SIX OCTETS À LA FUSION, MESURÉ : +1 057 → +1 051.**
`src/ui/monde.js` et `src/ui/raid.js` sont touchés par les DEUX côtés ; le
livrable fusionné n'est donc pas la somme des deux diffs, et c'est la mesure qui
le dit, pas l'arithmétique.
⚠⚠ **ET `main` EST ROUGE À L'HEURE OÙ CE LOT SE FUSIONNE, SUR LA GARDE DU COMPTE
DE TESTS.** Mesuré sur un `git worktree` pristine à `3e68d7b`, `dist/` rebâti :
**1542 déclarés · 1540 pass · 1 fail · 1 skipped**, et le rouge est
`documentation — CLAUDE.md §0 annonce le vrai nombre de tests` — sa §0 annonce
**1527**, le dépôt en déclare **1542**. Les trois lots du 10/09 ont chacun mesuré
leur compte contre `14dd4ac` et aucun n'a remesuré après l'atterrissage des deux
autres. **Ce lot-ci le referme** en écrivant 1550, qui est le compte mesuré de
l'arbre fusionné. ⚠ Même écart sur la taille : la §0 d'ART-90 annonce
**9 410 485**, l'arbre réel de `main` en rend **9 412 300** — les 1 815 octets
sont le JavaScript des deux lots frères, que sa mesure contre `14dd4ac` ne
portait pas. **Son bloc n'est pas réécrit** : il dit ce que son lot a mesuré, et
c'est ce qu'un bloc « Auparavant » doit dire.
Le lot touche `src/data/base.js`,
...
qui le mesure, et la marge est à 1,82 %. **Le levier existe et il est
...
`SAVE_VERSION` vaut **31** et **aucune ligne de ce lot ne le touche** — il est
passé de 30 à 31 sur `main`, par le lot RÈGLES-DE-CARTE, pas ici.
...
**Auparavant, après le lot ART-90 :**
`npm test` déclarait **1527** tests au sens de la garde de `documentation.test.js` ;
le verdict mesuré était **1526 pass · 0 fail ·
1 skipped** (`LIMITE T8`, suspendu par Ethan le 08/09), et `npm run check` sort
