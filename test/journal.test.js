...
import {
  ...
  COMBATS_DEPLACES_PAR_CONTACT,
  COMBATS_DEPLACES_PAR_CONTACT_AVANT_PAQUETS,
  COMBATS_DEPLACES_PAR_CONTACT_2,
...
test('JOURNAL T1 — deux cents combats rendent le résultat capturé au lot APPROCHE (falsification n° 1)', () => {
  ...
  for (const [nom, montage] of montagesTemoins(genererSite)) {
    ...
    // ⚠⚠ ET LE LOT CONTACT EN EMPILE UNE SECONDE, IL NE REMPLACE PAS LA
    // PREMIÈRE. Les couches se lisent de la plus ANCIENNE à la plus récente,
    // donc la dernière écrite l'emporte : ce que CONTACT déplace gagne, sinon
    // ce que BARÈME-ET-REJEU déplaçait, sinon le témoin d'APPROCHE.
    const deplacesContact = COMBATS_DEPLACES_PAR_CONTACT[i] ?? {};
    // ⚠⚠ ET LE LOT CONTACT-2 EN EMPILE UNE TROISIÈME, IL NE REMPLACE AUCUNE DES
    // DEUX AUTRES. Même doctrine, même ordre de lecture : la plus récente
    // l'emporte, sinon CONTACT, sinon BARÈME-ET-REJEU, sinon le témoin d'APPROCHE.
    const deplacesContact2 = COMBATS_DEPLACES_PAR_CONTACT_2[i] ?? {};
    // ⚠⚠ ET LE LOT PRÉDILECTION EN EMPILE UNE QUATRIÈME, IL N'EN REMPLACE
    ...
    for (let c = 1; c < vu.length; c += 1) {
      ...
      if (Object.prototype.hasOwnProperty.call(deplacesContact, c)) {
        reference = deplacesContact[c];
        couvert = true;
      }
      if (Object.prototype.hasOwnProperty.call(deplacesContact2, c)) {
        reference = deplacesContact2[c];
        couvert = true;
      }
  ...
  //
  // ⚠⚠ LOT CONTACT (13/09) : LA SURCHARGE PASSE DE 1 089 À **1 125**, ET IL RESTE
  // **475 CHAMPS GARDÉS** contre la capture d'APPROCHE. La couche neuve déplace
  // **1 086 champs** — le nombre que le brief annonçait, au champ près — mais
  // **1 050 d'entre eux étaient DÉJÀ surchargés** par BARÈME-ET-REJEU : le compte
  // est l'UNION des deux couches, pas leur somme, et il ne monte donc que de
  // trente-six. ⚠ Le brief en déduisait 514 gardés par simple soustraction
  // `1 600 − 1 086` ; c'est la couche SEULE, pas l'empilement, et le nombre juste
  // est 475. Mesuré, pas recopié.
  //
  // ⚠⚠ LOT CONTACT-2 (13/09) : LA SURCHARGE PASSE DE 1 125 À **1 153**, ET IL
  // RESTE **447 CHAMPS GARDÉS** contre la capture d'APPROCHE. La couche neuve
  // déplace **907 champs sur 1 600** — mais **879 d'entre eux étaient DÉJÀ
  // surchargés** par CONTACT ou par BARÈME-ET-REJEU : le compte est l'UNION des
  // trois couches, pas leur somme, et il ne monte donc que de vingt-huit.
  //
  // ⚠⚠ ET **VINGT-TROIS COMBATS SONT INTACTS**, là où les deux couches d'avant
  // touchaient les deux cents. C'est la mesure qui dit que ce lot-ci ne
  // recompose rien : il ferme un chevauchement en travers et étale l'écrasement
  // sur quatre ticks, donc un combat où aucune pièce ne mord sur deux index et
  // où rien n'écrase ne bouge pas d'un bit. Le compte est asserté pour qu'une
  // couche qui s'élargirait en silence fasse tomber ce test.
  //
  // ⚠⚠ LOT PRÉDILECTION (13/09) : LA SURCHARGE PASSE DE 1 153 À **1 179**, ET IL
  // RESTE **421 CHAMPS GARDÉS** contre la capture d'APPROCHE. La couche neuve
  // déplace **1 074 champs sur 1 600** et en ajoute **VINGT-SIX** à l'union.
  //
  // ⚠⚠ ET VINGT-SIX N'EST PAS UN OU DEUX — C'EST LA MESURE QUI DIT QUE CE LOT NE
  // TOUCHE PAS LA FILE. Les six lots d'avant — ARRÊT, COLONNE, MUR,
  // BARÈME-ET-REJEU, CONTACT, CONTACT-2 — déplaçaient tous le DÉPLACEMENT, et
  // leurs couches le disaient en n'ajoutant qu'un ou deux champs neufs. Celui-ci
  // change QUI une pièce élit : **huit causes de fin sur 200 basculent**, contre
  // deux au lot CONTACT-2. Un lot qui prétendrait ne toucher qu'au déplacement et
  // rendrait ce compte-là mentirait, et c'est cette assertion qui le dirait.
  //
  // ⚠ ET SEPT COMBATS SEULEMENT SONT INTACTS, contre vingt-trois au lot
  // CONTACT-2 : la prédilection mord partout où une pièce avait plus d'une cible
  // valide à portée, c'est-à-dire presque partout.
  assert.equal(surcharges, 1179, `champs surchargés : ${surcharges}`);
  assert.notEqual(surcharges, 1153, 'la couche de PRÉDILECTION a disparu');
  ...
  assert.equal(champs - surcharges, 421, 'le compte des champs encore gardés a changé');
});
...
// empreintes retombent sur `TEMOINS_COMBAT_AVANT_PAQUETS` sous les SEPT
// couches empilées depuis JOURNAL-DE-COMBAT : 1 334 surchargés, 266 gardés,
// exactement le compte d'avant le lot. Si ce test tombe un jour, c'est le
// moteur ou la copie qui a bougé, pas le placement.
test('JOURNAL T1 bis — l\'ancien placement rejoué : 0 écart sous les sept couches empilées', () => {
  let i = 0;
  let champs = 0;
  let surcharges = 0;
  for (const [nom, montage] of montagesTemoins(genererSiteAncien, DEPART_AVANT_APPROCHE)) {
    {
      ...
      assert.equal(vu[0], attendu[0], `le témoin ${i} n'est pas dans l'ordre`);
          ...
          // ⚠⚠ ET LE LOT CONTACT EN AJOUTE UNE SEPTIÈME. Même doctrine, septième
          // fois : on empile, on ne remplace pas. ⚠ Elle déplace 1 051 champs et
          // n'en ajoute qu'**UN SEUL** à la surcharge — 1 050 étaient déjà
          // couverts par l'une des six d'avant. C'est moins encore que les DEUX
          // du lot BARÈME-ET-REJEU, et c'est la mesure qui dit que ce lot déplace
          // le même axe qu'ARRÊT, COLONNE, MUR et BARÈME-ET-REJEU : la FILE.
          const deplacesContact = COMBATS_DEPLACES_PAR_CONTACT_AVANT_PAQUETS[i] ?? {};
          // ⚠⚠ ET LE LOT CONTACT-2 EN AJOUTE UNE HUITIÈME. Même doctrine, huitième
          // fois : on empile, on ne remplace pas. ⚠ Elle déplace 764 champs et
          // n'en ajoute qu'**UN SEUL** à la surcharge — 763 étaient déjà couverts
          // par l'une des sept d'avant, exactement comme la couche du lot
          // CONTACT. C'est la mesure qui dit que ce lot déplace le même axe que
          // les cinq précédents — la FILE —, plus l'instant de la mort d'une
          // écrasée, et ni le tir ni le ciblage.
          const deplacesContact2 = COMBATS_DEPLACES_PAR_CONTACT_2_AVANT_PAQUETS[i] ?? {};
          // ⚠⚠ ET LE LOT PRÉDILECTION EN AJOUTE UNE NEUVIÈME. Même doctrine,
          // neuvième fois : on empile, on ne remplace pas. ⚠ Elle déplace 994
          // champs et n'en ajoute que **DEUX** à la surcharge — mais ici le
          // compte des NEUFS ne mesure plus grand-chose : huit couches empilées
          // ne laissent que 265 champs à couvrir, dont l'essentiel sont des
          // CAUSES de fin. La mesure qui compte est dans `T1`, où l'empilement
          ...
          for (let c = 1; c < vu.length; c += 1) {
            ...
            if (Object.prototype.hasOwnProperty.call(deplaces, c)
              ...
              || Object.prototype.hasOwnProperty.call(deplacesBareme, c)
              || Object.prototype.hasOwnProperty.call(deplacesContact, c)
              || Object.prototype.hasOwnProperty.call(deplacesContact2, c)
              || Object.prototype.hasOwnProperty.call(deplacesPredilection, c)) surcharges += 1;
          }
          i += 1;
    }
  }
  ...
  //
  // ⚠⚠ LOT CONTACT (13/09) : LA SEPTIÈME COUCHE TOUCHE LES DEUX CENTS COMBATS ET
  // DÉPLACE 1 051 CHAMPS, ET LA SURCHARGE NE MONTE QUE D'**UN SEUL** — de 1 333
  // à **1 334**, les gardés de 267 à **266**. Les 1 050 autres étaient DÉJÀ
  // surchargés par l'une des six d'avant : le compte est l'UNION des sept, pas
  // leur somme. ⚠ C'est **moins encore que les DEUX** du lot BARÈME-ET-REJEU, et
  // c'est l'invariance la plus forte que ce test ait mesurée : borner le pas au
  // contact déplace le MÊME axe qu'ARRÊT, COLONNE, MUR et BARÈME-ET-REJEU — la
  // FILE. Un lot qui aurait touché au tir ou au ciblage aurait fait sauter les
  // 266, qui sont pour l'essentiel des CAUSES de fin ; trois seulement bougent.
  //
  // ⚠⚠ LOT CONTACT-2 (13/09) : LA HUITIÈME COUCHE DÉPLACE **764 CHAMPS** ET LA
  // SURCHARGE MONTE DE **1 334 À 1 335**, LES GARDÉS DE 266 À **265**. Un seul
  // champ neuf, comme au lot CONTACT : **763 des 764 étaient déjà couverts** par
  // l'une des sept d'avant. Sixième lot d'affilée à n'ajouter qu'un ou deux
  // champs à l'union, et c'est la mesure qui dit que ce lot déplace le même axe
  // que les cinq précédents — la FILE —, plus l'instant de la mort d'une
  // écrasée, et ni le tir ni le ciblage : ceux-là auraient fait sauter les 265,
  // qui sont pour l'essentiel des CAUSES de fin. **Deux seulement bougent.**
  //
  // ⚠ ET CINQUANTE ET UN COMBATS SONT INTACTS ICI, CONTRE VINGT-TROIS DANS `T1` :
  // l'ancien placement fait naître les vagues sur le front de la bande de
  // déploiement, donc moins de pièces se croisent en travers en approchant, donc
  // la fenêtre élargie mord moins souvent.
  //
  // ⚠⚠ LOT PRÉDILECTION (13/09) : LA NEUVIÈME COUCHE DÉPLACE **994 CHAMPS** ET LA
  // SURCHARGE MONTE DE **1 335 À 1 337**, LES GARDÉS DE 265 À **263**. Deux
  // champs neufs — mais **ne pas lire ce deux comme les DEUX du lot
  // BARÈME-ET-REJEU** : ici la pile fait huit couches et il ne restait que 265
  // champs à couvrir. La mesure comparable est celle de `T1`, dont la pile n'en
  // porte que trois : **vingt-six champs neufs**, et **huit causes de fin sur 200
  // qui basculent** contre **trois ici**.
  //
  // ⚠ ET CETTE ASYMÉTRIE-LÀ EST UN FAIT DU PLACEMENT, PAS DU LOT : l'ancien
  // placement fait naître les vagues sur le front de la bande de déploiement,
  // donc les pièces arrivent plus groupées et le choix de cible discrimine moins.
  // Dix combats sont intacts ici, sept dans `T1`.
  assert.equal(surcharges, 1337, `champs surchargés : ${surcharges}`);
  assert.notEqual(surcharges, 1335, 'la couche de PRÉDILECTION a disparu');
  ...
  assert.equal(
    ...
    149, 'la couche de CONTACT-2 a changé de largeur');
  assert.equal(
    Object.values(COMBATS_DEPLACES_PAR_PREDILECTION_AVANT_PAQUETS)
      .filter((d) => Object.keys(d).length > 0).length,
    190, 'la couche de PRÉDILECTION a changé de largeur');
  assert.ok(champs - surcharges === 263, 'le compte des champs encore gardés a changé');
});
...
test('JOURNAL T8 — l\'encaissé est publié avec les PV max de la cible (falsification n° 8)', () => {
  ...
  //
  // ⚠⚠ LOT PRÉDILECTION (13/09) : LA GRAINE NE BOUGE PAS, LE PLAFOND DE LA BOUCLE
  // SI — 400 → **450**, ET C'EST UN RESSERREMENT. Le combat durait 303 ticks ;
  // il en dure **416**, parce que les attaquantes élisent désormais la cible de
  // leur prédilection au lieu de la plus proche, donc elles tuent mieux et
  // vivent plus longtemps. À 400 la boucle TRONQUAIT le combat, et un test qui
  // mesure un combat tronqué mesure le PLAFOND, pas le combat — c'est la faute
  ...
  let vus = 0;
  let parts = [];
  const ecartsNonPublies = [];
  while (!etat.termine && ticks < 450) {
    ...
    for (const i of etat.journal.impacts) {
      ...
      // ⚠⚠ LOT CONTACT-2 : L'EXCEPTION SE RELÈVE AU LIEU DE S'AUTORISER, ET
      // C'EST UN RESSERREMENT. `e.ecrase` n'est posé qu'À LA MORT ; l'écrasement
      // dure désormais QUATRE ticks, donc une pièce qui perd son quart pendant
      // les trois premiers n'a pas encore le drapeau, et le `if` d'avant ne la
      // couvrait plus — mesuré, c'est exactement par là que ce test est tombé.
      // On ne désarme donc pas la garde : on RELÈVE chaque écart, et on le juge
  ...
  // ⚠⚠ ET LE COMBAT DOIT ÊTRE FINI : une boucle qui sort par son plafond mesure
  // le plafond. 416 ticks mesurés ; le plafond est à 450.
  assert.ok(etat.termine,
    `le combat n'est pas fini au tick ${ticks} : la boucle sort par son plafond `
    + 'et ne mesure plus le combat — relever le plafond et remesurer, pas assouplir');
  ...
  //
  // ⚠⚠ LOT CONTACT (13/09) : LE COMPTE REDESCEND DE TROIS À DEUX, ET CE QUI
  // DISPARAÎT A ÉTÉ MESURÉ AVANT D'ÊTRE RÉPARÉ — pièce par pièce, contre le
  // livrable pristine de `main` = `48827c8` :
  //
  //                                    AVANT              APRÈS
  //   Carapace ATTAQUANTE, ind. 79     tick 64            tick 64      ← identique
  //   Guetteur de garnison, ind. 28    tick 164           tick 155
  //   Guetteur de garnison, ind. 24    tick 216           JAMAIS
  //   impacts publiés                  1 243              1 324
  //   ticks de combat                  304                281
  //
  // ⚠⚠ ET LE GUETTEUR 24 NE MEURT PLUS PARCE QU'IL S'ARRÊTE AU CONTACT — c'est
  // le lot qui travaille, pas un écrasement perdu. Relevé tick par tick : il se
  // décale de la colonne 9 000 vers le bas ; sous l'ancienne règle il FLUAIT
  // jusqu'à **3 360**, donc dans la case 3, où une attaquante l'écrase au tick
  // 216. Sous la neuve il **s'immobilise à 4 000** — le contact exact, une case
  // pleine de son bloqueur — du tick 200 au tick 217, l'attaquante passe en
  // colonne 3 sans le trouver, puis il repart et finit le combat **vivant**,
  // à 18 612 657 milli-PV.
  //
  // ⚠⚠ ET CE QUI MESURE L'EXCEPTION NE BOUGE PAS D'UN MILLIÈME : la Carapace
  // attaquante, indice 79, est écrasée au **même tick 64**, rangée 3 780,
  // colonne 2 000 des deux côtés. Les DEUX exceptions — « touchée ET écrasée au
  // même tick », donc `encaissé < perdu` — restent exercées : 351 861 encaissés
  // pour 19 989 875 perdus sur la Carapace, 282 947 pour 25 700 498 sur le
  // Guetteur 28. La garde du dessus continue donc de servir.
  //
  // ⚠⚠ L'ASSERTION PORTE DÉSORMAIS SUR LES INDICES, PAS SUR UN COMPTE — ET
  // C'EST UN RESSERREMENT, PAS UN RÉANCRAGE. Le compte de DEUX est celui
  // d'avant le lot BARÈME-ET-REJEU, que son `notEqual` refusait de face : deux
  // mécanismes sans rapport rendent le même nombre, et un `equal(…, 2)` nu
  // laisserait passer un lot qui rouvrirait le chevauchement allié. La LISTE,
  // elle, les distingue — `[24, 28, 79]` sous le chevauchement rouvert,
  // `[28, 79]` sous la règle du contact — et elle NOMME la pièce qui a changé
  // de sort au lieu de compter.
  //
  // ⚠⚠ LOT CONTACT-2 (13/09) : `[28, 79]` → **`[10, 20, 21, 28]`**, ET LE COMPTE
  // MONTE DE DEUX À QUATRE. Mesuré pièce par pièce contre le livrable pristine de
  // `main` = `f21ba4e` :
  //
  //                                    AVANT              APRÈS
  //   Carapace ATTAQUANTE, ind. 79     tick 64            JAMAIS
  //   Guetteur de garnison, ind. 28    tick 155           tick 164
  //   Carapace de garnison, ind. 10    —                  tick 222
  //   ind. 20 et 21                    —                  écrasés
  //   impacts publiés                  1 324              1 314
  //   ticks de combat                  281                303
  //
  // ⚠⚠ ET LA CARAPACE 79 NE MEURT PLUS ÉCRASÉE PARCE QU'ELLE MEURT AUTREMENT :
  // l'écrasement prend quatre ticks au lieu d'un, donc une victime a le temps de
  // tomber sous le TIR pendant qu'on l'écrase, et c'est alors une mort ordinaire.
  // C'est le lot qui travaille, pas un écrasement perdu — voir `CONTACT-2 T1`,
  // qui mesure les quatre ticks sur un montage où rien d'autre ne tire.
  //
  // ⚠⚠ LOT PRÉDILECTION (13/09) : `[10, 20, 21, 28]` → **`[10, 11]`**, ET LE
  // COMPTE REDESCEND DE QUATRE À DEUX. Mesuré pièce par pièce contre le livrable
  // pristine de `main` = `0c4a546` :
  //
  //                                    AVANT              APRÈS
  //   Carapace de garnison, ind. 10    tick 222           tick 252
  //   Carapace de garnison, ind. 11    —                  tick 280
  //   Ratisseurs 20 et 21              écrasés            JAMAIS
  //   Guetteur de garnison, ind. 28    tick 164           JAMAIS
  //   impacts publiés                  1 314              1 444
  //   ticks de combat                  303                416
  //
  // ⚠⚠ ET CE QUI DISPARAÎT TOMBE SOUS LE TIR AU LIEU DE TOMBER SOUS L'ÉCRASEUR.
  // C'est le lot qui travaille : une attaquante qui élit la cible de sa
  // prédilection tue ce qu'elle est faite pour tuer, donc la garnison meurt
  // avant d'être broyée. L'écrasement est la mort de ce qui survit au tir ; il
  // en survit moins.
  //
  // ⚠⚠ ET L'EXCEPTION EST MIEUX EXERCÉE QU'AVANT, PAS MOINS : les DEUX écrasées
  ...
  // ⚠⚠ ET L'EXCEPTION EST JUGÉE ICI, SUR UNE CARACTÉRISATION POSITIVE QUI PEUT
  // TOMBER — l'idiome de `DETTES_ACCENT`, et un RESSERREMENT sur le `if` d'avant.
  // Un impact publie ce que la cible a ENCAISSÉ ; une pièce peut perdre DAVANTAGE
  // le même tick, et il n'y a qu'une façon que ça arrive : l'ÉCRASEMENT, qui
  // retire `ceil(pvMax / ECRASEMENT_TICKS)` à l'étape 7, hors de
  // `appliquerDegats`, donc sans publier d'impact. Trois choses sont donc exigées
  // de CHAQUE écart, et aucune n'est un `ok` de complaisance : la pièce finit
  // ÉCRASÉE, l'excès vaut AU PLUS un quantum d'écrasement, et le compte est exact.
  //
  // ⚠ MESURÉ : **cinq écarts sur 1 314 impacts, deux pièces** — la Carapace 10
  // sur QUATRE ticks consécutifs (219 à 222), dont les trois premiers à l'excès
  // EXACT du quantum (13 252 800) et le dernier au reste (8 973 481), et le
  // Guetteur 28 sur un seul tick. Les deux autres écrasées n'encaissent aucun tir
  // pendant leur écrasement, donc elles ne publient aucun impact à comparer.
  // Sur l'arbre d'avant, les mêmes trois exigences tenaient sur DEUX écarts d'un
  // tick chacun : la garde n'est pas relâchée, elle est dite.
  //
  // ⚠ LOT PRÉDILECTION : **huit écarts sur 1 444 impacts, deux pièces**, et les
  ...
  for (const x of indicesEnEcart) {
    assert.ok(ecrasees.includes(x),
      `la pièce ${x} perd plus que son impact et n'est pas écrasée : le journal ment`);
  }
  for (const e of ecartsNonPublies) {
    assert.ok(e.perdu - e.encaisse <= e.quantum,
