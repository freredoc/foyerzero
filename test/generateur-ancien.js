// L'ANCIEN PLACEMENT — celui d'AVANT le lot PAQUETS, 09/09/2026 —, copié ICI,
// jamais dans `src/`. Ce n'est pas un test, c'est le témoin de non-régression
// du MOTEUR.
//
// ⚠⚠ POURQUOI IL EXISTE. Le lot PAQUETS réécrit le placement d'un site, donc
// tout combat change, donc les deux cents empreintes de `temoins-combat.js`
// tombent — et le fichier dit qu'il ne se rafraîchit pas, parce qu'un témoin
// régénéré par la même main que le code suit l'erreur qu'il devrait attraper.
// La seule façon de recapturer sans perdre la preuve est de prouver d'abord que
// `src/sim/combat.js` n'a pas bougé : on rejoue l'ANCIEN placement — ce
// fichier —, on passe ses montages au moteur d'AUJOURD'HUI, et on exige que
// les deux cents anciennes empreintes repassent, à l'octet, couches de
// surcharge comprises. C'est `JOURNAL T1 ancien` qui le fait, à chaque
// `npm run check`. Si ce test tombe, c'est le MOTEUR qui a bougé, pas le
// placement.
//
// ⚠⚠ C'EST UNE COPIE, ET ELLE NE SUIT PAS `src/`. Les six fonctions du modèle
// ligne/colonne — `taillesDeRangee`, `contigueDepuisLOrigine`,
// `profilRealisable`, `profilDeCharge`, `repartirLesColonnes`,
// `placementDesRangees` — n'existent plus dans `src/sim/generateur.js` ; les
// helpers de composition (`composerRepartition`, `auPlusGrandReste`,
// `renormaliser`…) y existent encore, à l'identique, et sont recopiés ici
// parce qu'ils n'y sont pas exportés — leur ouvrir une porte pour un test
// serait une porte que la production n'emploie pas. Les cinq réglages retirés
// de `DISPOSITION_DEFENSES` sont figés dans `ANCIENNE_DISPOSITION`, aux valeurs
// qu'ils avaient sur `main` à `598d23a`.
//
// ⚠ NE PAS « CORRIGER » CE FICHIER : il doit rester ce qu'il était.

/** Les cinq réglages de `DISPOSITION_DEFENSES` retirés par le lot PAQUETS. */
const ANCIENNE_DISPOSITION = {
  versLeFond: true,
  ecartColonnesMax: 2,
...
import { enEntier, cleCase, estDansLaBande } from '../src/sim/grille.js';
 ...
 */
function repartitionInterpolee(table, niveau) {
 ...
 */
function composerBatiments(nbBatiments) {
 ...
 */
const SEL_PLACEMENT_DES_RANGEES = 8;
...
function placerBatiments(rng, placement, liste, niveau) {
  ...
  const tailles = taillesDeRangee(
    ...
    ANCIENNE_DISPOSITION.brassagesDeRangee, 'bâtiments',
  );
  const charge = profilDeCharge(
    rng, proportionnels.length, tailles,
    ANCIENNE_DISPOSITION.ecartColonnesMax, ANCIENNE_DISPOSITION.brassagesDeCharge, plancher,
  );
  ...
  const offsets = placementDesRangees(
    placement, lignes.length, fond - premiere, ANCIENNE_DISPOSITION.etalementMaxRangees,
  );
  let rang = 0;
...
function placerDefenses(rng, placement, liste, niveau) {
  ...
  if (nb === 0) return [];
  const tailles = taillesDeRangee(
    rng, nb, parRangee, rangeesMax, ANCIENNE_DISPOSITION.brassagesDeRangee, 'défenses',
  );
  const charge = profilDeCharge(
    rng, nb, tailles,
    ANCIENNE_DISPOSITION.ecartColonnesMax, ANCIENNE_DISPOSITION.brassagesDeCharge, null,
  );
  const lignes = repartirLesColonnes(rng, tailles, charge);
  ...
  // retourne pas.
  const offsets = placementDesRangees(
    placement, lignes.length, rangeesMax, ANCIENNE_DISPOSITION.etalementMaxRangees,
  );
  const poses = [];
  let i = 0;
  lignes.forEach((cols, j) => {
 ...
 */
function rangeeLaPlusAvanceeQuiTire(id) {
 ...
 */
export function genererSiteAncien({ type, niveau, saveur = null, graine }) {
