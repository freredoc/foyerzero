...
test('PIC T7 — le livrable pèse 10 611 332 octets, la marge sur la borne T10 est de 1,93 %', () => {
  ...
  //
  // ⚠⚠ ET LA BASE A BOUGÉ SOUS LE LOT : `main` EST PASSÉ DE `d68c4d1` À
  // `6eccedc` PENDANT SON EXÉCUTION, Ethan ayant fusionné EMPRISES-ET-DÉLAI
  // (#128) puis son correctif de CI (#129). Les DEUX lots avaient remesuré
  // contre `d68c4d1`, chacun de son côté, et **leurs gains portent sur des
  // postes DISJOINTS** — les images là-bas, la feuille ici. Le livrable
  // fusionné est donc REMESURÉ poste par poste contre la base neuve, jamais
  // recopié ni additionné : le lot ÉCRANS a déjà payé cette leçon-là, où le
  // JavaScript avait perdu six octets à la fusion sans qu'aucune arithmétique
  // le prévoie. ⚠ **ET LA MESURE DIT QUE LE COÛT DU LOT NE BOUGE PAS D'UN OCTET
  // À LA FUSION** — contre `6eccedc` comme contre `d68c4d1` : **feuille −97 361 ·
  // JavaScript +2 735 · balisage +400 · images +0 · audio +0**, total **−94 226**,
  // et les cinq postes PARTITIONNENT le fichier des deux côtés. C'est mesuré, pas
  // déduit : les deux lots ne partagent aucun fichier du livrable, là où ÉCRANS et
  // SON-ET-ARRIVÉE partageaient `src/ui/monde.js` et `src/ui/raid.js`.
  // ⚠ Le remesurage confirme au passage la ventilation d'EMPRISES-ET-DÉLAI par un
  // chemin indépendant : entre `d68c4d1` et `6eccedc`, les images tombent de
  // 7 651 104 à **7 630 256** (−20 848, l'atlas `batiment-128`) et le JavaScript
  // monte de 402 212 à **402 617** (+405, la table des cinquante plafonds).
  // ⚠ Et le numéro de version suit : `main` avait DÉJÀ pris
  ...
  // ⚠⚠ ET LE LOT BARÈME-ET-REJEU LE RÉANCRE À SON TOUR, MÊME JOUR, POUR LA MÊME
  // RAISON. Il coûte **+521 octets**, soit un centième de la tolérance de
  // 50 000 : la laisser tomberait très exactement sous la dérive LENTE que la
  // dernière assertion de ce test existe pour refuser, et le pourcentage annoncé
  // par `CLAUDE.md` cesserait d'être celui du disque. **Deux réancrages le même
  // jour, et c'est ce qu'on lui demande** — un lot qui sait ce qu'il déplace le
  // réécrit, quelle que soit la marge qui lui reste.
  // ⚠ VENTILÉ POSTE PAR POSTE contre le livrable rebâti dans un `git worktree`
  // sur l'arbre pristine de `main` = `f6fb04e` (**9 383 149**) : **JavaScript
  ...
  // dérive LENTE que la dernière assertion de ce test existe pour refuser.
  //
  // ⚠⚠ RÉANCRÉ AU LOT FREIN — SEPTIÈME FOIS, ET LE COÛT EST DE 593 OCTETS.
  // `main` pristine à `d6732fa` rend **9 386 114**, le nombre que la §0
  // annonçait, retrouvé à l'octet dans un `git worktree` ; l'arbre du lot rend
  // **9 386 707**. Ventilé poste par poste contre ce pristine-là : **JavaScript
  // +593 · feuille +0 · balisage +0 · images +0 · audio +0**, la partition
  // tombant EXACTEMENT sur le total des DEUX côtés — écart **0 · 0** —, et
  // **306 URI / 307 lignes `data:` de part et d'autre**. Le lot ne touche
  // qu'`src/sim/combat.js` : la borne NE BOUGE PAS.
  //
  // ⚠ ET 593 OCTETS VALENT UN QUATRE-VINGT-QUATRIÈME DE LA TOLÉRANCE DE 50 000 :
  ...
  // MESURE, pas du patch.
  // ⚠⚠ RÉANCRÉ AU LOT RUINES-DÉFENSE, 19/09, ET LA BORNE BOUGE AVEC — PREMIÈRE
  // FOIS DEPUIS ART-90. Huit ruines de pièce de défense entrent dans l'atlas
  // `defense`, qui passe de 18 à 26 sprites. Mesuré poste par poste contre le
  // livrable rebâti dans un `git worktree` sur l'arbre pristine de `main` =
  // `105d4fc`, qui est le merge d'ANCRES-ZÉNITH (**9 409 289**) : **images
  // +75 208 · JavaScript +898 · feuille +0 · balisage +0 · audio +0**, la
  // partition tombant EXACTEMENT sur le total des DEUX côtés — écart **0 · 0**
  // —, et **306 URI / 307 lignes `data:` de part et d'autre**.
  //
  // ⚠⚠ ET C'EST LE PLANCHER DE 150 000 QUI A FORCÉ LE RELÈVEMENT, PAS LA BORNE.
  // À 9 600 000 la marge tombait à **114 605 octets, 1,19 %** : le livrable
  // passait sous la borne, et l'assertion du bas de ce test ne passait pas. Une
  // ressource entre légitimement — huit dessins neufs, pas de l'entropie —, donc
  // la borne monte à **9 700 000** et `banc.test.js` T10 écrit pourquoi.
  //
  ...
  //
  // ⚠⚠ RÉANCRÉ AU LOT ARTILLERIE-RECHERCHE, 23/09/2026, ET C'EST LE MÊME MOTIF
  // QU'AU LOT PRÉCÉDENT — LE POURCENTAGE, PAS LA TOLÉRANCE. Le lot coûte
  // **+1 506 octets, ENTIÈREMENT DU JAVASCRIPT** : mesuré poste par poste contre
  // le livrable rebâti dans un `git worktree` pristine de `main` = `40c83a3`,
  // qui EST le merge du lot ARTILLERIE (**10 605 304**, retrouvé à l'octet),
  // **images +0 · audio +0 · JavaScript +1 506 · feuille +0 · balisage +0**, les
  // cinq postes PARTITIONNANT le fichier des deux côtés — écart **0 · 0** — et
  // `data:` à **316 lignes / 315 URI** de part et d'autre. Aucune image n'entre :
  // le lot ouvre une PORTE, il ne dessine rien.
  //
  // ⚠⚠ 1 506 OCTETS VALENT UN TRENTE-TROISIÈME DE LA TOLÉRANCE DE 50 000, DONC
  // LE LAISSER AURAIT PASSÉ AU VERT. Ce qui l'interdit reste la LIGNE D'EN
  // DESSOUS : la marge s'arrondit à **1,97 %** et non plus à 1,98 %, et elle se
  // calcule sur DEUX CONSTANTES — elle serait restée verte en annonçant un
  // pourcentage que le disque ne rend plus, et la §0 de `CLAUDE.md` écrirait
  // l'autre. C'est la troisième fois de suite que ce test est réancré par sa
  // décimale plutôt que par sa tolérance.
  //
  // ⚠⚠ RÉANCRÉ AU LOT BARRES-ET-RÉPARER, 25/09/2026, ET C'EST ENCORE LE
  // POURCENTAGE, PAS LA TOLÉRANCE. Le lot coûte **+1 595 octets, SANS UN OCTET
  // D'IMAGE NI DE SON** : mesuré poste par poste contre le livrable rebâti dans
  // un `git worktree` pristine de `main` = `78af575`, qui EST le merge du lot
  // ARTILLERIE-RECHERCHE (**10 606 810**, retrouvé à l'octet), **JavaScript +894
  // · feuille +685 · balisage +16 · images +0 · audio +0**, les cinq postes
  // PARTITIONNANT le fichier des deux côtés — écart **0 · 0** — et `data:` à
  // **316 lignes / 315 URI** de part et d'autre. Les seize octets de balisage
  // sont ` class="repliee"`, posé sur `#offense-tout-reparer`.
  //
  // ⚠⚠ 1 595 OCTETS VALENT UN TRENTE ET UNIÈME DE LA TOLÉRANCE DE 50 000, DONC
  // LE LAISSER AURAIT PASSÉ AU VERT — et la marge s'arrondit à **1,96 %**, plus
  // à 1,97 %. Quatrième fois de suite que ce test est réancré par sa décimale.
  //
